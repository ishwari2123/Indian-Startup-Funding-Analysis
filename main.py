...
# Drop the 'Remarks' column due to a high number of missing values, if it exists
if 'Remarks' in df.columns:
    df = df.drop('Remarks', axis=1)

# Handle missing values in 'Industry Vertical', 'SubVertical', 'City  Location',
# 'Investors Name', and 'InvestmentnType' by filling with 'Unknown'
...
# Handle inconsistent formats in 'Startup Name', 'Industry Vertical', 'SubVertical', 'City  Location', 'Investors Name', and 'InvestmentnType'
for col in ['Startup Name', 'Industry Vertical', 'SubVertical', 'City  Location', 'Investors Name', 'InvestmentnType']:
    df[col] = df[col].astype(str).str.strip().str.replace('\\xc2\\xa0', '', regex=True)  # Remove non-breaking spaces and trim
...
#8
plt.figure(figsize=(14, 7))
sns.barplot(data=investment_type_summary_by_deals.head(10),x='InvestmentnType', y='number_of_deals', palette='viridis')
